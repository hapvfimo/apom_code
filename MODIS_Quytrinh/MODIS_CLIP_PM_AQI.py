...
AQI_TCQT_TABLE = "prodaqi.prodmodisaqitcqt_vn_collection0"
AQI_TCVN_TABLE = "prodaqi.prodmodisaqitcvn_vn_collection0"
...
PMDIR = "/home/apom/apom/prod/ProdMODISPM_Province/2018/"
AQITCVN = "/home/apom/apom/prod/ProMODISAQI/tcvn/2018/"
AQITCQT = "/home/apom/apom/prod/ProMODISAQI/tcqt/2018/"
PMVNTABLE = "prodpm.prodmodispm_vn_collection0" # 2010 - 2014
PMPROVTABLE = "prodpm.prodmodispm_province_collection0"
SHAPEPROV = "org.gis_adm1"
AQITCVNVN = "prodaqi.prodmodisaqitcvn_vn_collection0"
AQITCVNPROV = "prodaqi.prodmodisaqitcvn_province_collection0"
AQITCQTVN = "prodaqi.prodmodisaqitcqt_vn_collection0" 
AQITCQTPROV = "prodaqi.prodmodisaqitcqt_province_collection0"
...
update_query1 = "UPDATE {0} SET rasref='{1}'::raster, filename='{2}', filepath='{3}', gridid={4}, aqstime='{5}'::timestamp, updatetime=now()::timestamp  WHERE type={6}" # tablename, rastertext, filename, satellite type (0-MOD, 1-MYD)
...
class Server:
	...
	# init
	#def __init__(self, host="118.70.72.13", dbname="apom_data", user="postgres", passwd="fimopostgre54321", port=8100 ):
	def __init__(self, host=host_local, dbname=db_local, user=user_local, passwd=pass_local, port=port_local):
		self.con = None
		self.host = host
		self.dbname = dbname
		self.user = user
		...
		self.con2 = None
		self.host2 = host_ubun
		self.dbname2 = db_ubun
		self.user2 = user_ubun
		self.passwd2 = pass_ubun
		self.port2 = port_ubun
		self.cur2 = None
	...
	def Connect(self):
		try:
			self.con = psycopg2.connect(database=self.dbname, user=self.user, password=self.passwd, host=self.host, port=self.port)	
			self.cur = self.con.cursor()
			self.con2 = psycopg2.connect(database=self.dbname2, user=self.user2, password=self.passwd2, host=self.host2, port=self.port2)	
	...
	def InsertTIFAQI(self, table, fname, fdir, oriid, grid_id, id_shape, aqstime, sat_type, typeinsert, sourceid):
		...
		if typeinsert == "PROV":
			sate_type = sat_type
			insertsql = insert_aqiprov.format(table, fname, fdir_store, oriid, grid_id, maxval, minval, avg, aqstime, raster, id_shape, sate_type, sourceid) # insert province
		#print insertsql
		try:
	...
	def InsUpdAQIVietnamAndProvinces(self, pmfile, aqidir, aqifile, oripmtable, oriaqitable, aqiprovtable, shapetable, typeaqi, sat_type, sourceid):
		try:
			...
			sql = "SELECT id FROM {0} WHERE filename = '{1}'".format(oripmtable, pmbasename)
			idpm = ""

			self.cur.execute(sql)
			idpm = self.cur.fetchall()
			# insert AQI full to database
			oripminfo = "SELECT id, gridid, aqstime FROM {0} WHERE filename='{1}'".format(oripmtable, pmbasename)
			print oripminfo
	...
	def GetPmFilename(self, pmtable):
		sql = "SELECT filename, to_char(aqstime, 'yyyy'), type, sourceid FROM {0} WHERE id = 537 or id = 539 ORDER BY aqstime asc".format(pmtable)
...
#pmname = "MOD04_L2.A2015097.0410.051.2015097135603_uk.tif"
#sat_type = 0
#sourceid = 0
#ExecuteAll(pmname, 0, 0)

#
# Truoc: 1 - AQUA, 0 - TERRA
# Sau:   1 - Tram thu, 0 - NASA
# 
ExecuteAll(sys.argv[1], sys.argv[2],sys.argv[3])
#pm_name = "MOD04_L2.A2018032.0240.006.2018032134902_uk.tif"
#sate_type =  0
#source_id =  0

#ExecuteAll(pm_name, sate_type, source_id)
#ExecuteAll("MOD04L2.A2017160.0306.006.2017160031504_uk.tif", 0, 1)
#ExecuteAll("MOD04L2.A2016308.0407.006.2016308041558_uk.tif", 0, 1)
...
def ExecuteAll():	
	...
	for pmname in records:
		...
		if d == 1:
			...
			continue
		PMDIR = "/apom_data/apom/prod/ProdMODISPM_Province/" + pmname[1] + "/"
		global AQITCVN # call the global first
		AQITCVN = "/apom_data/apom/prod/ProMODISAQI/tcvn/" + pmname[1] + "/"

		global AQITCQT # call the global first
		AQITCQT = "/apom_data/apom/prod/ProMODISAQI/tcqt/" + pmname[1] + "/"
